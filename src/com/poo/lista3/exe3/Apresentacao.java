...
public class Apresentacao extends javax.swing.JFrame {
	...
	private void initComponents() {
		...
		tfData.addFocusListener(new FocusAdapter() {
			...
			public void focusLost(FocusEvent e) {
				LocalDate dataDigitada = LocalDate.parse(tfData.getText(), formatter);
				if (!agenda.dataFoiAdicionada(dataDigitada)) {
					jBIncluirNaAgenda.setEnabled(true);
					return;
				}
				jBIncluirNaAgenda.setEnabled(false);
				DataAgenda dia = agenda.busca(dataDigitada);
				String msg = "Compromissos de alta prioridade: " + dia.getQtdCompromissosPrioridade('A');
				for (Compromisso c : dia.getCompromissos().values()) {
					msg += "\n"+c.exibir();
				}
	...
	private void jBIncluirNaAgendaActionPerformed(java.awt.event.ActionEvent evt) {
		...
		dia.setData(LocalDate.parse(tfData.getText(), formatter));
